...
stage: 备赛期 self-iteration + 飞书接入推进 + Skill 协议层收敛
stage_goal: 完善 debug-checklist skill 自用迭代 + 推进飞书 D-022 三包拆分（LARK-CLI-01..06）+ 跑通用户线下 onboard §0-§5 + 收敛 SKILL.md 协议 v1.0（SKILL-PROTOCOL-V1）
current_task: null  # SKILL-PROTOCOL-V1 闭环；LARK-CLI T1-T6 仍待用户线下走 onboard §0-§5
frontier: []
blocked:
...
_无。SKILL-PROTOCOL-V1（design）**已闭环**：协议本体 `.agents/skills/PROTOCOL-v1.0.md`（8 节，frontmatter 7 字段 + body 8 必填 H2 + 4–5 可选 H2 + extensions 扩展钩子 + 双 SemVer 版本号机制）+ `docs/planning/skill-protocol-migration-gap.md`（3 个 active 业务 skill 均评 B 级合规，工作量 ~20 min/skill；流程类 atomic-task 列 out-of-scope）+ `docs/design/D-023-skill-protocol-v1.md`（详细 ADR 草稿 status: draft，4 个放弃方案）+ `decisions.md` D-023 聚合段；7 项验证全过含 `verify:skills-sync` exit 0。**不动**三个现有 SKILL.md（迁移留 SKILL-MIGRATION-V1-* 系列后续任务，待 D-023 升 DECIDED 后认领）。LARK-CLI 系列 T1-T6 仍全部闭环，等用户线下走 onboard §0-§5。_
