...
//   apps/server/src/database.mjs         ← SQLite store
...
export async function startProbeFlashServer(overrides = {}) {
  ...
  const closeoutRecoveryScan = runCloseoutRecoveryScan(store, defaultWorkspace, overrides);

  const server = createServer(
    createRequestHandler({ store, storeInitError, staticDir, releaseMetadata }),
  );
